...
namespace Discord_Bot.Modules.MinecraftModule
{
    internal class MinecraftUtil
    {
        ...
        public MinecraftServerStatus GetServerStatus(bool useFullMotd = false)
        {
            ...
            {
                logger.LogError($"Status requested for invalid IP & Port '{targetHostString}:{targetPort}'");
                throw new InvalidOperationException("The Host and Port entered were invalid.");
            }
            if (!portIsValid)
            {
                logger.LogError($"Status requested for invalid Port '{targetHostString}:{targetPort}'");
                throw new InvalidOperationException("The Port entered was invalid.");
            }
            if (!(hostAsDomainIsValid || hostAsIPIsValid))
            {
                logger.LogError($"Status requested for invalid Host '{targetHostString}:{targetPort}'");
                throw new InvalidOperationException("The Host entered was invalid.");
            }

            logger.LogInformation($"Status requested for '{targetHostString}:{targetPort}'");
